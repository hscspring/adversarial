...
def get_args():
    return ADict({
        ...
        "lr_min": 0.0,
        "lr_max": 0.2, 
        "weight_decay": 0.01,   ## 5e-4,
        "momentum": 0.9, 
        "epsilon": 16,          ## 8
        "alpha": 10,
        "delta_init": "random", # zero, random, previous
    })
...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    mu = config.embedding_pretrained.mean(0, keepdims=True)
    std = config.embedding_pretrained.std(0, keepdims=True)
    upper_limit = ((1 - mu)/ std)
    lower_limit = ((0 - mu)/ std)
    base = config.n_vocab
    epsilon = (ad_args.epsilon / base) / std
    alpha = (ad_args.alpha / base) / std
    ...
    # optimizer = torch.optim.SGD(params, lr=ad_args.lr_max, momentum=ad_args.momentum, weight_decay=ad_args.weight_decay)
    optimizer = torch.optim.Adam(params, lr=config.learning_rate, weight_decay=ad_args.weight_decay)

    criterion = nn.CrossEntropyLoss()

    if ad_args.delta_init == 'previous':
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            if ad_args.delta_init == 'random':
                for j in range(epsilon.size(1)):
                    delta[:, :, j].uniform_(-epsilon[0][j].item(), epsilon[0][j].item())
            ...
            if total_batch % 100 == 0:
                ...
                if dev_loss < dev_best_loss:
                    ...
                    torch.save(model.state_dict(), config.save_path + "fgsm.ckpt")
                    torch.save(embedding.state_dict(), config.save_embedding_path + "fgsm.ckpt")
                    improve = '*'
...
def test(config, embedding, model, test_iter):
    # test
    model.load_state_dict(torch.load(config.save_path + "fgsm.ckpt"))
    model.eval()
    start_time = time.time()
